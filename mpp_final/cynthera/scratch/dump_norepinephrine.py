...
async def dump_norepinephrine():
    orchestrator = MasterOrchestrator()
    hyp, pkg, res = await orchestrator.evaluate(drug_name="Norepinephrine", disease_name="Heart Failure", policy=RetrievalPolicy.STANDARD, bypass_cache=False)
    ...
    print("\n--- NOREPINEPHRINE TARGETS RETRIEVED FROM CHEMBL ---")
    ...
    for t in pkg.targets:
        ...
        print(f"  Target: {t.name if hasattr(t, 'name') else 'N/A'} (UniProt: {uni}) -> Canonical: {canon} | Mechanism: {t.mechanism} -> Norm: {normalize_drug_action(t.mechanism).value}")

    print("\n--- NOREPINEPHRINE OPEN TARGETS DoE RECORDS ---")
    print(f"Total DoE Records: {len(pkg.opentargets_doe_evidence)}")
    for doe in pkg.opentargets_doe_evidence:
        print(f"  Target: {doe.target_symbol} (ID: {doe.target_id}) | LoF/GoF: {doe.direction_on_target} | Trait: {doe.direction_on_trait} | Datasource: {doe.datasource_id} | Score: {doe.score}")

    print("\n--- NOREPINEPHRINE DATTS RECORDS ---")
    ...
    print("\n--- NOREPINEPHRINE DRUGMECHDB RECORDS ---")
    ...
    print("\n--- NOREPINEPHRINE NORMALIZED THERAPEUTIC DIRECTION EVIDENCE (TDE) ---")
    ...
    print("\n--- NOREPINEPHRINE TARGET ALIGNMENTS IN REPORT ---")
...
if __name__ == "__main__":
    asyncio.run(dump_norepinephrine())
